          ...
          <div className="w-full max-w-xl group relative">
             <div className="flex items-center gap-4">
                ...
                <button 
                  ...
                  className={`p-5 rounded-2xl border transition-all duration-500 shadow-xl ${
                    showFilters ? "bg-white text-brand-blue border-white" : "bg-white/10 text-white border-white/10 hover:bg-white/20"
                  }`}
                >
                    ...
                    <div className="p-2 flex flex-wrap justify-center border border-white/10 bg-[#0c2160]/95 backdrop-blur-2xl rounded-2xl shadow-[0_30px_60px_rgba(0,0,0,0.5)]">
                      {categories.map((cat) => (
                        <button
                          ...
                          className={`px-6 py-3.5 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${
                            activeCategory === cat ? "bg-white text-brand-blue shadow-lg scale-105" : "text-white/60 hover:text-white"
                          }`}
                        >
                          {cat}
                        </button>
      ...
      <div className="max-w-[1500px] mx-auto px-6 lg:px-12 relative z-10">
        <div className={`flex flex-col ${isEven ? 'lg:flex-row' : 'lg:flex-row-reverse'} items-center gap-20 lg:gap-32`}>
          ...
          <div className="w-full lg:w-1/2">
             <motion.div initial={{ opacity: 0, x: isEven ? -40 : 40 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }} className="relative group">
                <div className="relative aspect-square sm:aspect-[4/3] rounded-[4rem] bg-white border border-gray-100 shadow-[20px_40px_80px_rgba(0,0,0,0.06)] overflow-hidden flex items-center justify-center p-4 transition-transform duration-700 hover:scale-[1.02]">
                   ...
                   <motion.img 
                     ...
                     onError={() => {
                       ...
                       else if (imgSrc.endsWith('.png')) setImgSrc(imgSrc.replace('.png', '.jpg'));
                     }}
                     alt={product.name}
                     whileHover={{ scale: 1.05 }}
                     transition={{ duration: 0.8 }}
                     className="w-full h-full object-contain z-10 p-8 sm:p-14"
                   />
                   <div className="absolute top-10 left-10 w-10 h-10 border-t-2 border-l-2 border-gold-dark/30" />
                   <div className="absolute bottom-10 right-10 w-10 h-10 border-b-2 border-r-2 border-gold-dark/30" />
                </div>

                <div className="absolute -bottom-8 right-6 lg:right-16 bg-white px-10 py-6 rounded-3xl shadow-2xl border border-gray-50 flex items-center gap-4">
                   <div className="w-12 h-12 rounded-full bg-brand-blue/5 flex items-center justify-center">
                      <ShieldCheck className="w-6 h-6 text-brand-blue" />
                   </div>
                   <div>
                      <p className="text-[9px] uppercase font-black text-brand-blue/40 tracking-[0.2em] mb-1">Clinic Core</p>
